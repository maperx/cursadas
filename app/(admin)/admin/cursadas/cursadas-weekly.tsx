...
export function CursadasWeekly({
  ...
  const dateStrByDay = new Map<number, string>();
  DAYS.forEach((day, i) => dateStrByDay.set(day, toDateStr(dayDates[i])));
            ...
            {DAYS.map((day) => (
              ...
              >
                ...
                {cursadasByDay.get(day)!.map((cursada) => {
                  ...
                  const occurrenceDate = dateStrByDay.get(day)!;
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`absolute right-1 left-1 overflow-hidden rounded-md border p-1.5 text-xs ${canEdit(cursada.aula.sedeId, cursada.examen) ? "cursor-pointer hover:brightness-95" : ""} transition-[filter] ${isSuspended ? "opacity-60" : ""}`}
                        style={{
                          top,
                          height,
